...
class UNet(nn.Module):
    def __init__(self, inChannels, outChannels):
        ...
        self.conv = conv2d(
            in_planes=n_feat, out_planes=outChannels, batch_norm=False,
            activation=nn.Tanh(), kernel_size=3, stride=1
        )
...
class SFE(nn.Module):
    def __init__(self, inChannels, outChannels):
        ...
        self.sfe = nn.Sequential(*[
            nn.Conv2d(inChannels, outChannels, kernel_size=5, padding=2, stride=1),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
...
class skip_connection_cat(nn.Module):
    def __init__(self, inChannels, outChannels, preChannels):
        ...
        self.conv1 = conv2d(
            in_planes=inChannels + preChannels, out_planes=outChannels,
            batch_norm=False, activation=nn.LeakyReLU(), kernel_size=5, stride=1
        )
        self.conv2 = conv2d(
            in_planes=outChannels, out_planes=outChannels, batch_norm=False,
...
class Decoder_layer(nn.Module):
    def __init__(self, inChannels, outChannels, preChannels):
        ...
        self.resblocks = Cascade_resnet_blocks(in_planes=outChannels, n_blocks=2)
        self.conv = conv2d(
            in_planes=outChannels, out_planes=outChannels,
            batch_norm=False, activation=nn.LeakyReLU(), kernel_size=3, stride=1
        )
...
class down(nn.Module):
    def __init__(self, inChannels, outChannels, filterSize):
        ...
        self.conv1 = conv2d(
            in_planes=inChannels, out_planes=outChannels,
...
class ImagePred(nn.Module):
    def __init__(self, inChannels):
        ...
        self.conv1 = conv2d(
            in_planes=inChannels, out_planes=inChannels,
            batch_norm=False, activation=nn.LeakyReLU(),
            kernel_size=3, stride=1
        )
        self.att = ECA(inChannels)
        self.conv2 = conv2d(
            in_planes=inChannels, out_planes=3,
            batch_norm=False, activation=nn.Sigmoid(),
            kernel_size=3, stride=1
        )
...
class PredFlow(nn.Module):
    def __init__(self, inChannels, scale):
        ...
        self.att = ECA(inChannels)
        self.conv2 = conv2d(
            in_planes=inChannels, out_planes=4,
    ...
    def __init__(self, in_planes, out_planes, batch_norm, activation, kernel_size=3, stride=1):
        ...
        modules.append(
            nn.Conv2d(
                ...
                padding=(kernel_size - 1) // 2, bias=use_bias
            )
        )
        if batch_norm:
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, in_planes):
        ...
        conv_block += [
            conv2d(
                in_planes=in_planes, out_planes=in_planes,
                batch_norm=False, activation=nn.ReLU(),
                kernel_size=3, stride=1
            )
        ]
        conv_block += [
