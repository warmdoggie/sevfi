    ...
    for i in range(len(opt.test_list)):
        ...
        if opt.dataset == 'SEID':
            ...
            testDataset = test_MVSEC_sevfi(data_path=test_path, num_bins=15, num_skip=opt.num_skip,
                                           num_insert=opt.num_insert)
        result_path = os.path.join(opt.save_path, 'insert_' + str(opt.num_insert), opt.test_list[i]) + '/'
        utils.mkdir(result_path)
        disp_path = os.path.join(opt.save_path, 'disp_' + str(opt.num_insert), opt.test_list[i]) + '/'
